...
pai = pd.read_excel('PAI.xlsx',
                    index_col = 0,
                    usecols = ['id', 'entidad', 'proyecto'])
...
for i in pai.index:
    ...
    pai['nouns'] = pai['word_types'][i].count('NN')
    pai['verbs'] = pai['word_types'][i].count('VBP')
    pai['adjectives'] = pai['word_types'][i].count('JJ')
